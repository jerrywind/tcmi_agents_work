...
`docker-compose.yml` 默认只启动 `rrserver` + `nginx`（云端反代/TLS）。
...
```bash
# 1. 生成自签名证书到 certs/（仅用于本机测试，浏览器会报不安全属正常）
...
生产部署请把 `certs/` 换成真实证书（如 certbot 申请的 `fullchain.pem`/`privkey.pem`），
...
1. 准备 TLS 证书到 `rrserver/certs/{fullchain.pem,privkey.pem}`（可用 certbot）。
2. 复制并填写配置：
   ```bash
...
- rrserver 仅监听内网，由 nginx 做 TLS 与对外暴露；不要在公网直接暴露 rrserver 端口。
