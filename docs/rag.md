# RAG 检索服务（llm_server 内置）

`llm_server/` 在提供 OpenAI 兼容模型服务的同时，内置一套 **Python 3** 实现的 RAG 服务，
覆盖三类检索：

1. **文本 RAG**：文本片段 → Embedding → 向量最近邻；
2. **图像 RAG**：图像 → Qwen3-VL caption → Embedding → 图像向量域最近邻（以图搜图）；
3. **图文对应 RAG**：每条记录同时拥有「文本向量」与「图像向量」，可用文本或图像查询，
   跨两域联合召回（以文搜图 / 以图搜文）。

源码位于 `llm_server/rag/`，由 `entrypoint.sh` 在启动模型时于后台拉起（端口 `RAG_PORT`，
默认 `8080`）。RAG 服务复用本服务的 Embedding 端点与 `llm_vision` 的 Qwen3-VL 端点，
无需额外模型权重。
...
```
                ┌─────────────────────────── llm_server (容器) ───────────────────────────┐
                │                                                                          │
 文本/查询 ─────┼─► RAG 服务 (:8080) ──(text Embedding)──► llama-server :8000 /v1/embeddings│
 图片/查询 ─────┤                                      │                                   │
                │                            (image caption)│                                   │
                │                                       ▼                                       │
                │                            llm_vision :8000 /v1/chat/completions (Qwen3-VL)   │
                │                                       │                                       │
...
| `rag/embedders.py` | `TextEmbedder`（OpenAI 兼容 `/v1/embeddings`）、`ImageEmbedder`（Qwen3-VL 生成 caption 再嵌入） |
...
```json
{
  ...
  "image_caption": "舌色红，苔黄腻",        // Qwen3-VL 生成的 caption（可选）
...
| `RAG_VISION_BASE_URL` | `http://llm_vision:8000/v1` | 图像 caption 用的 Qwen3-VL 端点 |
| `RAG_VISION_API_KEY` | `sk-noauth` | 视觉端点 Key |
| `RAG_VISION_MODEL` | `Qwen3-VL-8B` | 视觉模型名 |
| `RAG_TOP_K` | `5` | 默认返回条数 |
...
后端可经 `TCM_RAG_BASE_URL` 指向本服务（compose `llm` profile 已配置
`TCM_RAG_BASE_URL=http://llm_server:8080`）。Sub-Agent 在执行时若需检索药典、医案、
舌象图谱等资料，可调用 RAG 服务获得相关上下文后再生成结论；无 RAG 服务时不影响现有
rule/LLM 流程（参见 `docs/sub_agents.md`、`docs/skills.md`）。
