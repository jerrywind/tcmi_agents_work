...
> - **流式**：带 `delta` 出口且 `config.llm_stream=true` 时逐 token 推给 `POST /chat/stream`
>   的 SSE 连接；上游不返回 SSE 则按实际 `Content-Type` 退回整包解析（不会静默产出空正文）。
>
...
- **实现**：
  1. **前置规则（结构化，`assess()` 纯函数，T4.1/T4.2 + 阶段 H 的 H1–H4）**：
     - **症状分级**：`key_symptoms`（主症）权重 **1.0**、`minor_symptoms`（次症）**0.4**
       （旧字段 `symptoms` 加载时并入次症）。此前所有表现同权，症状表越长的证候越容易赢；
     - 证据量 = 主/次症命中 + 舌象/脉象命中（短语按「，」拆段匹配）
       + 关键词证据 0.5 − 每条矛盾证据 0.5；
     - **去重**：关键词与任一已命中的症状/舌脉**互为子串**即视为同一表现，不再重复计分
       （此前「口苦」先按症状 +1、再按关键词 +0.5，证据量被放大约 1.5 倍）；
     - 置信度 = `min(1, 证据量 / 5)`，保留两位小数；
     - **矛盾证据**：命中表现的**相反表现**若出现在语料中即计入，
       相反表现表在 `resources/contradictions.yaml`（中医可维护）；
     - **三道闸（H3，最要紧）**：① 至少命中 1 条主症（`qualified`）；
       ② 证据量 ≥ 1.5（孤证不立）；③ **无合格候选则 `primary = null`**，
       并产出 `near`（最接近谁 + 缺哪条主症）交给模型与前端。
       ——此前 `assess()` 在库里**必然**产出主证，库外的证被硬塞成库内最像的那一个，
       且看起来有置信度；
     - 主证 = 合格候选中证据量最高者（同分保持证候库顺序）；
...
- ⚠️ **证据语料只取 `role=user` 的消息**（`model::user_corpus()`）。
  助手上一轮的总结会复述证名与症状（还带「若出现胸痛…请立即就医」这类警示语），
  把它当证据再算一遍会造成**自我确认**：第一轮猜个证 → 总结复述 → 第二轮证据变多 →
  置信度升 → 收敛提前通过。实测：患者只说「咳嗽，痰黄稠」时本应判「不知道」，
  加一段助手总结后变成「风热犯肺证 3.8」，5 条证据里 3 条是助手编的。
  `differentiation::assess`、`convergence::evaluate`、`inquiry::collected` 三处共用该函数。
...
- **与本地库的关系**：`formulas.yaml` 现有 **36 首**（覆盖 16 个证候，带 `source` 出处），
  是确定性兜底；真正的覆盖面来自方书检索。
  规则结果**注入 system**（T7.6），模型按库载方剂开方，输出再做确定性的药味比对（T7.4）。
...
| 想改什么 | 改哪里 | 是否需重启 |
|---|---|---|
| System Prompt（14 段） | `resources/prompts.yaml` | `POST /reload` 或重启 |
